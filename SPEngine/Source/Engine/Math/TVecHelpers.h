...
#ifndef SP_MATH_TVECHELPERS_H
#define SP_MATH_TVECHELPERS_H
...
template <typename T, typename B = int, typename C = int, typename D = int>
concept C_Arithmetic = is_arithmetic<T>::value && is_arithmetic<B>::value && is_arithmetic<C>::value && is_arithmetic<D>::value;
...
private:
     ...
     */
    template <typename U>
    friend inline constexpr bool MATH_PURE operator==(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
    ...
    template <typename U>
    friend inline constexpr VECTOR<bool> MATH_PURE equal(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
        VECTOR<bool> r{};
        for (size_t i = 0; i < lv.size(); i++) {
            r[i] = lv[i] == rv[i];
    ...
    template <typename U>
    friend inline constexpr VECTOR<bool> MATH_PURE notEqual(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
        VECTOR<bool> r{};
        for (size_t i = 0; i < lv.size(); i++) {
            r[i] = lv[i] != rv[i];
    ...
    template <typename U>
    friend inline constexpr VECTOR<bool> MATH_PURE lessThan(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
        VECTOR<bool> r{};
        for (size_t i = 0; i < lv.size(); i++) {
            r[i] = lv[i] < rv[i];
    ...
    template <typename U>
    friend inline constexpr VECTOR<bool> MATH_PURE lessThanEqual(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
        VECTOR<bool> r{};
        for (size_t i = 0; i < lv.size(); i++) {
            r[i] = lv[i] <= rv[i];
    ...
    template <typename U>
    friend inline constexpr VECTOR<bool> MATH_PURE greaterThan(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
        VECTOR<bool> r;
        for (size_t i = 0; i < lv.size(); i++) {
            r[i] = lv[i] > rv[i];
    ...
    template <typename U>
    friend inline VECTOR<bool> MATH_PURE greaterThanEqual(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
        VECTOR<bool> r{};
        for (size_t i = 0; i < lv.size(); i++) {
            r[i] = lv[i] >= rv[i];
...
private:
     ...
     */
    template <typename U>
    friend constexpr inline arithmetic_result_t<T, U> MATH_PURE dot(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
    ...
    friend inline T MATH_PURE norm(const VECTOR<T>& lv) { return std::sqrt(dot(lv, lv)); }

    friend inline T MATH_PURE length(const VECTOR<T>& lv) { return norm(lv); }

    friend inline constexpr T MATH_PURE norm2(const VECTOR<T>& lv) { return dot(lv, lv); }

    friend inline constexpr T MATH_PURE length2(const VECTOR<T>& lv) { return norm2(lv); }

    template <typename U>
    friend inline constexpr arithmetic_result_t<T, U> MATH_PURE distance(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
        return length(rv - lv);
    }

    template <typename U>
    friend inline constexpr arithmetic_result_t<T, U> MATH_PURE distance2(const VECTOR<T>& lv, const VECTOR<U>& rv)
    {
        return length2(rv - lv);
    }

    friend inline VECTOR<T> MATH_PURE normalize(const VECTOR<T>& lv) { return lv * (T(1) / length(lv)); }

    friend inline VECTOR<T> MATH_PURE rcp(VECTOR<T> v) { return T(1) / v; }
    ...
    template <typename U>
    friend inline VECTOR<T> MATH_PURE fmod(VECTOR<T> const& x, VECTOR<U> const& y)
    {
        VECTOR<T> r;
        for (size_t i = 0; i < r.size(); i++) {
    ...
    template <typename U>
    friend inline VECTOR<T> MATH_PURE remainder(VECTOR<T> const& x, VECTOR<U> const& y)
    {
        VECTOR<T> r;
        for (size_t i = 0; i < r.size(); i++) {
    ...
    template <typename U>
    friend inline VECTOR<T> MATH_PURE remquo(VECTOR<T> const& x, VECTOR<U> const& y, VECTOR<int>* q)
    {
        VECTOR<T> r;
        for (size_t i = 0; i < r.size(); i++) {
    ...
    friend inline constexpr VECTOR<T> MATH_PURE saturate(const VECTOR<T>& lv) { return clamp(lv, T(0), T(1)); }
