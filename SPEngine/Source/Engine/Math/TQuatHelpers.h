...
#ifndef SP_MATH_TQUATHELPERS_H
#define SP_MATH_TQUATHELPERS_H
...
public:
     ...
     */
    template <typename U>
    friend inline constexpr QUATERNION<arithmetic_result_t<T, U>> MATH_PURE operator*(const QUATERNION<T>& q,
                                                                                      const QUATERNION<U>& r)
    {
        // could be written as:
        //  return QUATERNION<T>(
...
public:
    ...
    template <typename U>
    friend inline constexpr arithmetic_result_t<T, U> MATH_PURE dot(const QUATERNION<T>& p, const QUATERNION<U>& q)
    {
        return p.x * q.x + p.y * q.y + p.z * q.z + p.w * q.w;
    }

    friend inline T MATH_PURE norm(const QUATERNION<T>& q) { return std::sqrt(dot(q, q)); }

    friend inline T MATH_PURE length(const QUATERNION<T>& q) { return norm(q); }

    friend inline constexpr T MATH_PURE length2(const QUATERNION<T>& q) { return dot(q, q); }
    ...
    friend inline constexpr QUATERNION<T> MATH_PURE conj(const QUATERNION<T>& q) { return QUATERNION<T>(q.w, -q.x, -q.y, -q.z); }

    friend inline constexpr QUATERNION<T> MATH_PURE inverse(const QUATERNION<T>& q) { return conj(q) * (T(1) / dot(q, q)); }

    friend inline constexpr T MATH_PURE real(const QUATERNION<T>& q) { return q.w; }

    friend inline constexpr TVec3<T> MATH_PURE imaginary(const QUATERNION<T>& q) { return q.xyz; }

    friend inline constexpr QUATERNION<T> MATH_PURE unreal(const QUATERNION<T>& q) { return QUATERNION<T>(q.xyz, 0); }
    ...
    friend inline QUATERNION<T> MATH_PURE slerp(const QUATERNION<T>& p, const QUATERNION<T>& q, T t)
    {
        ...
        const T a    = std::acos(SP::math::clamp(absd / npq, T(-1), T(1)));
        const T a0   = a * (1 - t);
        const T a1   = a * t;
        const T sina = sin(a);
        if (sina < value_eps) {
    ...
    friend inline constexpr QUATERNION<T> MATH_PURE positive(const QUATERNION<T>& q) { return q.w < 0 ? -q : q; }
};

}   // namespace SP::math::details

#endif   // SP_MATH_TQUATHELPERS_H
