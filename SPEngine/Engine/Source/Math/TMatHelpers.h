...
#ifndef SP_MATH_TMATHELPERS_H
#define SP_MATH_TMATHELPERS_H
...
{
    static constexpr T determinant(Matrix<T, 2> in) { return in[0][0] * in[1][1] - in[0][1] * in[1][0]; }
};
...
template <typename MATRIX, typename VEC,
          typename = std::enable_if_t<MATRIX::NUM_ROWS == MATRIX::NUM_COLS && (VEC::SIZE + 1) == MATRIX::NUM_COLS, int>>
inline constexpr MATRIX MATH_PURE translate(const MATRIX& m, const VEC& v)
{
    // for now we only handle square matrix transpose
    MATRIX result = m;
    if constexpr (MATRIX::NUM_COLS == 3) {
        result[2] = m[0] * v[0] + m[1] * v[1] + m[2];
    }
    if constexpr (MATRIX::NUM_COLS == 4) {
...
template <typename MATRIX, typename VEC,
          typename = std::enable_if_t<MATRIX::NUM_ROWS == MATRIX::NUM_COLS && (VEC::SIZE + 1) == MATRIX::NUM_COLS, int>>
inline constexpr MATRIX MATH_PURE scale(const MATRIX& m, const VEC& v)
{
    MATRIX result;
    if constexpr (MATRIX::NUM_COLS == 3) {
        result[0] = m[0] * v[0];
...
template <typename MATRIX, typename = std::enable_if_t<MATRIX::NUM_ROWS == MATRIX::NUM_COLS, int>>
inline constexpr MATRIX MATH_PURE scale(const MATRIX& m, typename MATRIX::value_type s)
{
    MATRIX result;
    if constexpr (MATRIX::NUM_COLS == 3) {
        result[0] = m[0] * s;
...
template <typename MATRIX, typename VEC, typename T = MATRIX::value_type,
          typename = std::enable_if_t<
              std::is_same_v<typename MATRIX::value_type, float> || std::is_same_v<typename MATRIX::value_type, double>, int>,
          typename = std::enable_if_t<MATRIX::NUM_ROWS == MATRIX::NUM_COLS && (VEC::SIZE + 1) == MATRIX::NUM_COLS, int>>
inline constexpr MATRIX MATH_PURE rotate(const MATRIX& m, T radian, const VEC& v)
{
    T const a = radian;
    T const c = std::cos(a);
    T const s = std::sin(a);

    VEC axis(normalize(v));
    VEC temp((T(1) - c) * axis);
...
template <template <typename> class BASE, typename T, template <typename> class VEC>
...
private:
    ...
    //  matrix * matrix
    template <typename U>
    friend inline constexpr BASE<arithmetic_result_t<T, U>> MATH_PURE operator*(BASE<T> lhs, BASE<U> rhs)
    {
...
private:
     ...
     * (the first one, BASE<T> being known).
     */
    friend inline constexpr BASE<T> MATH_PURE inverse(const BASE<T>& matrix) { return matrix::inverse(matrix); }

    friend inline constexpr BASE<T> MATH_PURE cof(const BASE<T>& matrix) { return matrix::cof(matrix); }

    friend inline constexpr BASE<T> MATH_PURE transpose(BASE<T> m) { return matrix::transpose(m); }

    friend inline constexpr T MATH_PURE trace(BASE<T> m) { return matrix::trace(m); }

    friend inline constexpr T MATH_PURE det(const BASE<T>& m) { return matrix::det(m); }
    ...
    friend inline constexpr auto MATH_PURE diag(const BASE<T>& m) { return matrix::diag(m); }
};
...
public:
    ...
    // array access
    constexpr T const* asArray() const { return &static_cast<BASE<T> const&>(*this)[0][0]; }

    // element access
    inline constexpr T const& operator()(size_t row, size_t col) const { return static_cast<BASE<T> const&>(*this)[col][row]; }

    inline T& operator()(size_t row, size_t col) { return static_cast<BASE<T>&>(*this)[col][row]; }
...
public:
    ...
    TQuaternion<T> toQuaternion() const { return matrix::extractQuat(static_cast<const BASE<T>&>(*this)); }
};
