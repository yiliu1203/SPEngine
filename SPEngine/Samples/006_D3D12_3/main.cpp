...
public:
        ...
        , mDev(nullptr)
    {
        ...
        CHK(mDxgiFactory->CreateSwapChainForHwnd(
            mCmdQueue.Get(), hWnd, &scDesc, nullptr, nullptr, mSwapChain.ReleaseAndGetAddressOf()));

        CHK(mDev->CreateCommandList(
            0, D3D12_COMMAND_LIST_TYPE_DIRECT, mCmdAlloc.Get(), nullptr, IID_PPV_ARGS(mCmdList.ReleaseAndGetAddressOf())));
...

        {
            ...
            mDev->CreateRootSignature(
                0, sig->GetBufferPointer(), sig->GetBufferSize(), IID_PPV_ARGS(mRootSignature.ReleaseAndGetAddressOf()));
            sig->Release();
        }
        ...
        ID3D10Blob *vs, *ps;
        {
            ...
            CHK(D3DCompileFromFile(ws::GetAssetFullPath("shader/default.hlsl").c_str(),
                                   nullptr,
                                   nullptr,
                                   "VSMain",
                                   "vs_5_0",
        ...
        float3         vbData[4]       = {{-0.7f, 0.7f, 0.0f}, {0.7f, 0.7f, 0.0f}, {-0.7f, -0.7f, 0.0f}, {0.7f, -0.7f, 0.0f}};
        unsigned short ibData[6]       = {0, 1, 2, 2, 1, 3};
        auto           HEAP_PROPERTIES = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
        auto           buff            = CD3DX12_RESOURCE_DESC::Buffer(sizeof(vbData) + sizeof(ibData));
        CHK(mDev->CreateCommittedResource(&HEAP_PROPERTIES,
    ...
    ~D3D() { CloseHandle(mFenceEveneHandle); }
