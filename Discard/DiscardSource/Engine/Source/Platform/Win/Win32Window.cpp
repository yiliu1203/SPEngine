...
LRESULT Win32Window::HandleMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
    ...
    case WM_DESTROY:
    {
        Global::g_input->OnCloseMsg();
        PostQuitMessage(0);
        break;
    }
    case WM_KILLFOCUS:
    {
        Global::g_input->OnLoseFocus();
        break;
    }
    ...
    case WM_KEYDOWN:
    {
        ...
        Global::g_input->OnKeyPressed(static_cast<int>(wParam), isReapeat);
        break;
    }
    case WM_KEYUP: Global::g_input->OnKeyReleased(static_cast<int>(wParam)); break;
    case WM_CHAR: Global::g_input->OnChar(static_cast<int>(wParam)); break;
    case WM_MOUSEMOVE:
    {
        // todo when mouse drag outof window
        const POINTS pt = MAKEPOINTS_FROM_PARAM(lParam);
        Global::g_input->OnMouseMoved(pt.x, pt.y);
        break;
    }
    ...
    {
        const POINTS pt = MAKEPOINTS_FROM_PARAM(lParam);
        Global::g_input->OnMouseLeftButtonPressed(pt.x, pt.y);
        break;
    }
    ...
    {
        const POINTS pt = MAKEPOINTS_FROM_PARAM(lParam);
        Global::g_input->OnMouseRightButtonPressed(pt.x, pt.y);
        break;
    }
    ...
    case WM_MOUSEWHEEL:
    {
        ...
        Global::g_input->OnMouseScrolled(pt.x, pt.y, delta);
        break;
    }
